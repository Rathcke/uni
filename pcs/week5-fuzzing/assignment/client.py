...
class Client(Thread):
        ...
        while self.running:
            ...
            try:
                if   s.startswith('MSG') and self.onMsg:
                    _typ, _tok, t1, t2, nick, msg = s.split(' ', 5)
                    ts = time.strptime(t1 + ' ' + t2, '%Y-%m-%d %H:%M:%S')
                    self.onMsg(nick, ts, msg)
                elif s.startswith('TOPIC') and self.onTopic:
                    _typ, _tok, t1, t2, nick, topic = s.split(' ', 5)
                    ts = time.strptime(t1 + ' ' + t2, '%Y-%m-%d %H:%M:%S')
                    self.onTopic(nick, ts, topic)
    ...
    def do_join(self, channel):
        self.sendline('JOINING #%s' % channel)
        s = self.recvthis('TOKEN')
        self.token = s[6:]

    def do_part(self):
        self.sendline('PART %s' % self.token)
        self.recvthis('LEFT OK')

    def do_quit(self):
        self.sendline('QUIT PLEASE')
...
class GUI(Thread):
    def __init__(self):
        ...
        write('*** Yet Another Internet Relay Chat ***')
        ...
        goto(15, self.height / 2 + 2)
        self.channel = raw_input('Channel [pcs2016]: ') or 'pcs2016'
        clearscreen()
        self.retopic()
        self.restatus()
        self.client.do_join(self.channel)
        self.help()
        self.start()
        try:
            while True:
    ...
    def run(self):
        ...
        while self.running:
            ...
            if n % 10: # roughly every second
                ...
                self.users = sorted(ansi_escape.sub('', u) for u in users)
                status = ' #%s' % self.channel + time.strftime(' (%H:%M) ')
                if self.users == []:
                    users = 'Forever alone :\'('
                else:
    ...
    def on_topic(self, nick, ts, topic):
        self.topic = topic
        save()
        self.retopic()
        restore()
        flush()
        self.msg('%s changed topic to %s' % (nick, topic))
