  ...
  *)
  ...
  fun typeCheckExp( vtab, AbSyn.Literal(AbSyn.BVal v, pos), _ ) =
    ...
    | typeCheckExp( vtab, AbSyn.LValue( AbSyn.Index(id, inds), pos ), _ ) =
      let val new_inds = List.map (fn arg =>  typeCheckExp(vtab,arg, KnownType (BType (Int)))) inds
          val inds_tps  = List.map (fn arg => typeOfExp(arg)) new_inds
          val condition = List.foldl (fn (t1, b) => b andalso typesEqual(t1,BType (Int))) true (inds_tps)
          val length = (List.length inds)
          val id_newtp = case SymTab.lookup id vtab of 
                       SOME tp => tp
                      | _   => raise Error("Could not find variable in vtable"^id, pos)
          val (rank,id_tp) = case id_newtp of
                      Array(r,btp) => (r,Array(r,btp))
                      | _ => raise Error("Type error"^id,pos)

      in if (condition andalso (length = rank) andalso (rank > 0))
                  then LValue(Index ((id,id_tp), new_inds), pos)
          else raise Error( "Error in Array", pos)
      end
    ...
    | typeCheckExp( vtab, AbSyn.Plus (e1, e2, pos), _ ) =
        ...
        in if typesEqual(BType Int, tp1) andalso typesEqual(BType Int, tp2)
            ...
            else raise Error("in type check plus exp, one argument is not of type int" 
    ...
    | typeCheckExp( vtab, AbSyn.Minus (e1, e2, pos), _ ) =
        ...
        in if typesEqual(BType Int, tp1) andalso typesEqual(BType Int, tp2)
            ...
            else raise Error("in type check minus exp, one argument is not of type int" 
                              ^ pp_type tp1 ^ " and " ^ pp_type tp2 ^ " at ", pos)
        end
    ...
    | typeCheckExp ( vtab, AbSyn.Times (e1, e2, pos), _ ) =
        ...
        in if typesEqual(BType Int, tp1) andalso typesEqual(BType Int, tp2)
            ...
            else raise Error("in type check times exp, one argument is not of type int" 
                              ^ pp_type tp1 ^ " and " ^ pp_type tp2 ^ " at ", pos)
        end
    ...
    | typeCheckExp ( vtab, AbSyn.Div   (e1, e2, pos), _ ) =
        ...
        in if typesEqual(BType Int, tp1) andalso typesEqual(BType Int, tp2)
            ...
            else raise Error("in type check div exp, one argument is not of type int" 
                              ^ pp_type tp1 ^ " and " ^ pp_type tp2 ^ " at ", pos)
        end
    ...
    | typeCheckExp ( vtab, AbSyn.FunApp ("new", args, pos), etp ) =
        ( case expectedBasicType etp of
            SOME btp => 
                let val new_args = List.map (fn arg =>  typeCheckExp(vtab,arg, KnownType (BType (Int)))) args
                    val arg_tps  = List.map (fn arg => typeOfExp(arg)) new_args
                    val condition = List.foldl (fn (t1, b) => b andalso typesEqual(t1,BType (Int))) true (arg_tps)
                    val rtp = Array (length args, btp)
                in  
                  if condition
                   then FunApp(("new", (arg_tps, SOME rtp)), new_args, pos)
                   else raise Error("Match error", pos)
                end
