...
and callFun ( (rtp : Type option, fid : string, fargs : Dec list, body : StmtBlock, pdcl : Pos),
    ...
    in  case fid of
        ...
        | other    =>
          ...
          (***       2. after the executing the call, for ANY expression in `aexps',   ***)
          ...
          (*******************************************************************************)
            ...
            in  ( case (rtp, res) of
                    (NONE  , _     ) => #2 (ListPair.map (fn (out_exp,in_arg) =>
                                           updateOuterVtable vtab new_vtab (out_exp,in_arg) ) (aexps,List.drop(fargs,2)), NONE)
 ...
 *)
and updateOuterVtable vtabOuter vtabInner (LValue (Var (out_exp,_),_), Dec((in_arg,_),_)) = 
    (case (SymTab.lookup out_exp vtabOuter, SymTab.lookup in_arg vtabInner) of
       (SOME out_ref, SOME in_ref) => out_ref := !in_ref
     |   _                         => raise Error2("Could not find one of the variables in the vtable")

       )
  | updateOuterVtable vtabOuter vtabInner (_,_) =
    raise Error2("Arguments do not match expected arguments")
