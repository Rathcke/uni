...
struct
  ...
  (***************************************************************************)
    ...
    | compileExp( vtable, Times(e1, e2, pos), place ) =
        let val t1 = "mul1_" ^ newName()
            val c1 = compileExp(vtable, e1, t1)
            val t2 = "mul2_" ^ newName()
            val c2 = compileExp(vtable, e2, t2)
    ...
    | compileExp( vtable, Not(e1, pos), place ) =
        let val t0 = "not1_" ^ newName()
            val t1 = "const1_" ^ newName()
            val c0 = compileExp(vtable, e1, t0)
            val lN = "_not_" ^ newName()
        in
           c0 
           @ [Mips.ADDI(t1,"0","1"),Mips.XOR(place,t0,t1)]
           @ [Mips.LABEL lN]
        end
 ...
 *  but instead moving args back to registers. **)
  and putArgsBack [] vtable reg = []
    | putArgsBack  es vtable reg=
      let  
        val t1 = (counter := !counter - 1;"_funarg__" ^ Int.toString(!counter)^"_") 
        val code2 = putArgsBack es vtable (reg-1)
      in
          (  code2                           (* compute rest *)
           @ [Mips.MOVE (t1, makeConst reg)] (* store in reg *)
              )
      end
  ...
  and compileLVal( vtab : VTab, Var (n,_) : LVAL, pos : Pos ) : Mips.mips list * Location =
    ...
    | compileLVal( vtab : VTab, Index ((n,t),inds) : LVAL, pos : Pos ) =
        case t of Array(r,btp) =>
          case SymTab.lookup n vtab of 
           SOME memAdr =>
           let val strd_reg = "strd_reg_" ^ newName()
               val exp_reg = "exp_reg" ^ newName()
               val acc = "acc_" ^ newName()
               val acc_ok = "acc_ok_" ^ newName()
               val rank = Int.toString((length inds) * 4)
               val inds_cut = List.take (inds,(List.length inds - 1))
               val inds_last = List.nth (inds,List.length inds)
               val val_inds = List.foldl (fn (i,code) => 
                                  let
                                    val codeE = compileExp(vtab,i,exp_reg)
                                  in
                                     [Mips.LW(strd_reg,memAdr,rank), 
                                     Mips.MUL(strd_reg,strd_reg,exp_reg),
                                     Mips.ADD(acc, acc, strd_reg),
                                     Mips.LW(strd_reg,memAdr,"0"),
                                     Mips.SLTI(strd_reg,exp_reg,"0"),
                                     Mips.XORI(strd_reg,strd_reg,"1"),
                                     Mips.AND(acc_ok,acc_ok,strd_reg),
                                     Mips.SLT(strd_reg,exp_reg,strd_reg),
                                     Mips.AND(acc_ok,acc_ok,strd_reg),
                                     Mips.ADDI(memAdr,memAdr,"4")] @ code
                                  end
                                 ) [] inds_cut
              val codeE = compileExp(vtab,inds_last,exp_reg)
           in  ([Mips.ADD(acc,acc,exp_reg), Mips.LW(strd_reg,memAdr,"0")]
                @ [Mips.SLTI(strd_reg,exp_reg,"0"),Mips.XORI(strd_reg,strd_reg,"1")]
                @ [Mips.AND(acc_ok,acc_ok,strd_reg),Mips.SLT(strd_reg,exp_reg,strd_reg)]
                @ [Mips.AND(acc_ok,acc_ok,strd_reg),Mips.BNE(acc_ok,"0","_Dim_Fine_")]
                @ [Mips.J("_IllegalArrIndexError"),Mips.LABEL("_Dim_Fine_")] @ val_inds 
                @ [Mips.ADD(strd_reg,memAdr,rank), Mips.LW(strd_reg,strd_reg,"0")] @
                (case btp of
                    Int => [Mips.SLL(acc,acc,"4")] 
                  | _   => [Mips.SLL(acc,acc,"2")])
                @ [Mips.ADD(acc,acc,strd_reg), Mips.ADDI(strd_reg,"0",rank)]
                @ [Mips.SLL(strd_reg,strd_reg,"1"),Mips.ADD(acc,acc,strd_reg)], Reg acc)

           end
          | NONE => raise Error( "Array pointer is not in memory, at ", pos)
          | othertype => raise Error( "Is not an array, at ", pos)
  ...
  and compileStmt( vtab, ProcCall (("write",_), [e], pos), _ ) =
    ...
    | ProcCall ((n,_), es, p) => 
      let
          ...
          val (mvbcode) = putArgsBack es vtable maxreg
      in
         #2(counter:=  !counter + (List.length es),
   ...
   * i.e. something similar to 'argcode', just the other way round.  Use the
           ...
           * or a procedure. **)
          val argbackcode = map (fn (vname, reg) => Mips.MOVE (reg,vname)) movePairs
          val body = compileStmts block vtable (fname ^ "_exit")
          val (body1, _, maxr, spilled) = (* call register allocator *)
              RegAlloc.registerAlloc (if isProc then (argcode @ body @ argbackcode) else (argcode @ body )) 
