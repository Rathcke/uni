...
struct
  ...
  (***************************************************************************)
    ...
    (*** EXAM HINTS:                                          ***)
    (*** 1. MAP & ZIPWITH:                                    ***)
    (***    Allocate the header of the result array by        ***)
    (***       incrementing the heap pointer (HP)             ***)
    (***    Copy the metadata of an input array in the        ***)
    (***      result array, i.e., the dimensions and strides  ***)
    (***      and compute the product of the dimensions       ***)
    (***      of one input array, say it is named flat_size.  ***)
    (***    For zipWith, remember to check that the two input ***)
    (***      arrays have the same shape, otherwise ERROR     ***)
    (***    Allocate the content of the result array, i.e.,   ***)
    (***      `flat_size' elements; if result array element   ***)
    (***      type is Int, remember to multiply by 4 (bytes)  ***)
    (*** 2. Also for REDUCE                                   ***)
    (***    write a loop of count `flat_size' which           ***)
    (***    computes the result, i.e., sets each elements of  ***) 
    (***    the result array for map and zipWith, and         ***)
    (***    in a scalar for `reduce'                          ***)
    (*** Use `mkFunCallCode' to call a function on some       ***)
    (***    symbolic register actual args, see below          ***)
    (************************************************************)

    (************************************************************)
    (*** LOW=LEVEL EXAM HINTS:                                ***)
    (************************************************************)
    (*** REMEMBER: ALLOCATING an array of `n_bytes' bytes,    ***)
    (***             into register `place', where `n_bytes' is***)
    (***             a symbolic register, can be done with:   ***)
    (***                                                      ***) 
    (***     [ Mips.MOVE(place, HP),                          ***)
    (***       Mips.ADD (HP, HP, n_bytes) ]                   ***)
    (***                                                      ***)
    (***  i.e., `place' becomes the old heap pointer, and the ***)
    (***        heap pointer is incremented to the next free  ***)
    (***        space (by `n_bytes' bytes)                    ***)
    (************************************************************)
    (*** If you want to CALL a function of name `id' and      ***)
    (***    signature `signat' on one actual argument stored  ***)
    (***    in symbolic register `tmp_reg', and the result    ***)
    (***    is to be stored in `res_reg' then ONE CAN WRITE:  ***)
    (***                                                      ***)
    (*** mkFunCallCode( (id,signat),[tmp_reg],vtable,res_reg )***)
    (***                                                      ***)
    (***    where `vtable' is the variable symbol table       ***)
    (************************************************************)
    ...
    (***   1. f is the FIdent, i.e., (id,signat), of the          ***)
    (***        to-be-called function                             ***)
    (***   2. regps are the actual parameter (symbolic) registers ***)
    (***   3. vtab  is the variable sysmbol table                 ***)
    (***   4. regres is the register that will store the result of***)
    (***        the function call                                 ***)
    (*** Output:                                                  ***)
    (***   The machine code for calling function `f' on regps     ***)
    (****************************************************************)
  and mkFunCallCode(f : FIdent, regps : string list, vtab : VTab, regres : string) = 
        let val (fnm, (ptps, rtp)) = f
            val dummy_nms  = map  ( fn x => "var_"^newName() ) regps
            val dummy_vars = ListPair.map  ( fn (nm,tp) => LValue (Var (nm,tp), (0,0)) ) 
                                           (dummy_nms,ptps)
            val new_vtab   = ListPair.foldl( fn (v,r,tab) => SymTab.bind v r tab ) 
                                           vtab (dummy_nms, regps)
            val call_node  = FunApp(f, dummy_vars, (0,0))
        in  compileExp(new_vtab, call_node, regres)
        end
